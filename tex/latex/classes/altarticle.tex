...
\envline[\argt{thmref}\optargt{extra}][{[]}]{proof}This environment allows to create proofs, with \aargt{thmref} being the reference to a theorem created in the document. The text \aargt{extra} is to be added right after the reference to the theorem and is initially empty.
\envline[\argt{thmref}\optargt{extra}][{[]}]{preuve}This environment is the same as the \texttt{proof} one.
\subsection{Examples}
The code
\begin{verbatim}
...
\theorem*[euclid star theorem]['s theorem]{Euclid}For every prime $p$,
    there is a prime $p'>p$. In particular, there are infinitely many
    primes.
\begin{proof}{thm euclid star theorem}
    Lorem ipsum \dots
\end{proof}
\end{verbatim}
produces
